...
sudo -u kla bash -lc \
  "cd '$NEW' && set -a && source /etc/kla/edziennik.env && set +a && NODE_ENV=development npm_config_production=false npm ci --include=dev && npm run db:generate && npm run check && npm run build"

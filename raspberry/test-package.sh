...
grep -q 'clamav-daemon clamdscan' "$ROOT/raspberry/install.sh"
grep -q 'usb-storage.quirks=0bc2:ab26:u' "$ROOT/raspberry/install.sh"
...
grep -q 'runuser -u postgres -- pg_dump' "$ROOT/raspberry/backup.sh"
grep -q 'runuser -u postgres -- pg_restore' "$ROOT/raspberry/restore.sh"
! grep -q 'sudo -u' "$ROOT/raspberry/backup.sh"
! grep -q 'sudo -u' "$ROOT/raspberry/restore.sh"
grep -q 'pg_restore --no-owner --role=kla_app' "$ROOT/raspberry/restore.sh"
...
grep -q 'recovery-key-first-run-once' "$ROOT/raspberry/web-control-daemon.py"
grep -q 'backup-recovery-key-first-run-shown' "$ROOT/raspberry/print-recovery-key.sh"

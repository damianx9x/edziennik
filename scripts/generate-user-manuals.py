...
def parent_functions():
    return [
        manual_entry("Moje dzieci: dodawanie i hasło", "Dodaj dziecko bez skrzynki e-mail. Wybierz miejsce zajęć i grupę z list szkoły.", "Start rodzica → Moje dzieci", "rodzic-dzieci.png", ["zaloguj się na swoje konto rodzica", "sprawdź, czy dziecko nie ma już konta w szkole"], ["Na ekranie Start kliknij „Otwórz moje dzieci”.", "W formularzu „Dodaj dziecko” wpisz imię i nazwisko.", "Wybierz lokalizację, a następnie jedną z grup dostępnych w tym miejscu.", "Ustal hasło mające co najmniej 8 znaków i powtórz je. Możesz użyć przycisku „Pokaż hasła”.", "Kliknij „Dodaj dziecko”. Zapisz gotowy login lub użyj „Kopiuj login”. Łucja Żółć otrzyma np. lucjazolc; jeśli login jest zajęty, system doda numer.", "Dziecko może się zalogować od razu. Grupa, jej plan i materiały pojawią się po potwierdzeniu przez szkołę.", "Aby zmienić hasło, otwórz „Ustaw lub zmień hasło” przy dziecku i wpisz nowe hasło dwa razy."], "Lista pokazuje Twoje dzieci, ich loginy i potwierdzone grupy. Błąd formularza nie usuwa wpisanych danych.", ["dodać kolejne dziecko", "wybrać lokalizację i grupę", "zmienić hasło własnego dziecka"], ["przypisać sobie istniejącego ucznia po nazwisku", "zmienić hasło cudzej osoby", "odblokować konta zawieszonego przez szkołę"], ["brak grup: wybierz inne miejsce lub napisz do szkoły", "hasła się różnią: popraw powtórzenie hasła", "dziecko ma już konto: poproś szkołę o połączenie kont"], "Login wpisujemy bez spacji i polskich znaków. Przekaż login i hasło dziecku osobiście. Nie wysyłaj hasła na grupę."),
        manual_entry("Pierwsze logowanie i wybór dziecka", "Konto rodzica pokazuje Twoje dzieci oraz ich bieżące sprawy.", "Link z zaproszenia → logowanie → samouczek → Start", "rodzic-start.png", ["użyj własnego zaproszenia", "przygotuj hasło mające co najmniej 12 znaków", "nie zakładaj drugiego konta dla kolejnego dziecka"], ["Otwórz link z zaproszenia, ustaw hasło i potwierdź adres e-mail.", "Zaloguj się i przejdź krótki samouczek.", "Po samouczku wybierz „Pobierz PDF”, aby zachować ten podręcznik, albo „Nie teraz”.", "Na górze panelu wybierz dziecko, jeżeli szkoła przypisała ich kilkoro.", "Sprawdź, czy imię, grupa i najbliższe zajęcia są poprawne."], "Panel zapamiętuje wybrane dziecko i pokazuje tylko jego plan, naukę, postępy oraz dokumenty powiązane z rodzicem.", ["przełączać się między własnymi dziećmi", "zmienić własne hasło", "pobrać aktualny podręcznik"], ["zobaczyć dzieci innej rodziny", "użyć jednego zaproszenia do utworzenia kilku kont"], ["brak dziecka: napisz do dyrektora, aby poprawił przypisanie", "brak e-maila: sprawdź spam i poprawność adresu", "zły panel: wyloguj się z konta innej osoby"], "Jedno konto rodzica może obsługiwać kilkoro dzieci. Nie twórz osobnego konta dla każdego z nich."),
...
def build_owner_manual(path: Path):
    ...
    story += [
        ...
        *bullets(["systemd Restart=always dla aplikacji", "osobny restart tunelu Cloudflare", "minutowy healthcheck usług i bazy", "sprzętowy watchdog systemd", "trwały journal poprzedniego uruchomienia", "timery kopii, retencji, testu odtworzenia i kolejki e-mail"]),
        ...
        Paragraph("Po uruchomieniu Raspberry zaczekaj na dysk: system może czekać do 180 sekund. Audyt sprawdza także obecność systemd-cryptsetup, a watchdog ponawia montowanie i uruchamia panel sterowania. Jeżeli mimo tego jest błąd, nie formatuj dysku i nie zmieniaj kluczy. Sprawdź zasilanie dysku oraz pełny audyt startu.", S["body"]),

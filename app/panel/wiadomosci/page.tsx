  ...
  return <AuthenticatedPanelShell session={session} active="messages">
    ...
    {channels.length === 0 && !isManagement ? <section className="messaging-empty"><MessageCircleMore aria-hidden="true" /><h2>Nie masz jeszcze rozmowy</h2><p>Gdy szkoła przypisze Ci grupę lub doda do rozmowy, pojawi się tutaj automatycznie.</p></section> : <MessagingWorkspace
      ...
      messages={messages.map((message) => ({
        ...
        readByCurrent: message.reads.length > 0,
        acknowledgedByCurrent: message.acknowledgements.length > 0,
        reactionSummary: Object.entries(message.reactions.reduce<Record<string, { count: number; reacted: boolean }>>((summary, reaction) => {
          const current = summary[reaction.emoji] ?? { count: 0, reacted: false };
          summary[reaction.emoji] = { count: current.count + 1, reacted: current.reacted || reaction.userId === session.user.id };
          return summary;
        }, {})).map(([emoji, value]) => ({ emoji, ...value })),
        delivery: { sent: message.deliveries.filter((item) => item.status === "SENT").length, pending: message.deliveries.filter((item) => ["QUEUED", "SENDING"].includes(item.status)).length, failed: message.deliveries.filter((item) => item.status === "FAILED").length },
      }))}
      queueStats={Object.fromEntries(queueStats.map((item) => [item.status, item._count._all]))}
    />}
    <aside className={`messaging-privacy-note${selected ? " has-selection" : ""}`}><Radio aria-hidden="true" /><p><strong>To komunikator służbowy szkoły.</strong> Dyrektor ma wgląd w wiadomości. Każde otwarcie przez dyrektora jest automatycznie zapisywane w historii bezpieczeństwa. Nie wpisuj haseł ani danych medycznych.</p></aside>

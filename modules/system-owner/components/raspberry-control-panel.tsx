...
export function RaspberryControlPanel({ status, storage }: { status: RaspberryStatus; storage: StorageDevice[] }) {
  ...
  return <section className="raspberry-control" aria-labelledby="raspberry-title">
    ...
    <nav className="raspberry-quick-nav" aria-label="Szybkie przejście do ustawień serwera">
      <a href="#backup-usb"><HardDrive aria-hidden="true" /><span><strong>Backup</strong><small>Wykryj i wybierz dysk USB</small></span></a>
      <a href="#email-delivery"><Mail aria-hidden="true" /><span><strong>Wysyłka e-mail</strong><small>{status.emailConfigured ? "SMTP jest skonfigurowane" : "SMTP wymaga konfiguracji"}</small></span></a>
      <a href="#full-export"><Download aria-hidden="true" /><span><strong>Pełny eksport</strong><small>Pobierz zaszyfrowaną kopię</small></span></a>
    </nav>
    ...
    <div className="raspberry-config-grid">
      <article id="backup-usb" className="integration-card owner-config-target"><header><Usb /><div><span className="section-kicker">Wykrywanie USB</span><h3>Dodatkowy dysk backupu</h3></div></header>
      ...
      <article id="email-delivery" className="integration-card owner-config-target"><header><Mail /><div><span className="section-kicker">Wysyłka w imieniu szkoły</span><h3>Serwer wysyłki e-mail</h3></div><span className={`integration-status ${status.emailConfigured ? "ready" : "planning"}`}>{status.emailConfigured ? <><CheckCircle2 aria-hidden="true" /> Aktywna</> : <><Mail aria-hidden="true" /> Do ustawienia</>}</span></header>
      ...
      <article id="full-export" className="integration-card owner-config-target"><header><Download /><div><span className="section-kicker">Awaryjne przeniesienie szkoły</span><h3>Pełny eksport systemu</h3></div></header>

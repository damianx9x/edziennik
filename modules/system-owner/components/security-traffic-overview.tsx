import { MapPinned, Radar, ShieldAlert, Smartphone } from "lucide-react";
...
export function SecurityTrafficOverview({ protectedActivity, regionActivity, deviceActivity, polandVisits, foreignVisits, unknownVisits }: {
  ...
  return <section className={`owner-security-overview ${critical.length ? "has-critical" : ""}`} aria-labelledby="owner-security-title">
    ...
    <div className="owner-security-grid">
      <article className="owner-security-card"><header><MapPinned aria-hidden="true" /><div><span className="section-kicker">Przybliżona mapa Polski</span><h3>Skąd otwierano stronę</h3></div></header><div className="poland-traffic-summary"><strong>{polandVisits}</strong><span>wejść z Polski · {foreignVisits} spoza Polski · {unknownVisits} bez danych kraju</span></div><div className="poland-region-map" aria-label="Przybliżona liczba wejść według województwa">{regions.map(([code, name, area]) => { const visits = byCode.get(code)?.visits ?? 0; return <div key={code} style={{ gridArea: area, "--traffic": visits / maxRegion } as React.CSSProperties} className={visits ? "has-data" : undefined} title={`${name}: ${visits}`}><abbr title={name}>{code}</abbr><strong>{visits}</strong></div>; })}</div><p className="owner-security-note">Region pojawi się, gdy Cloudflare przekaże nagłówek województwa. Bez tej informacji system nie zgaduje lokalizacji.</p></article>
      <article className="owner-security-card"><header><ShieldAlert aria-hidden="true" /><div><span className="section-kicker">Ostatnie 24 godziny</span><h3>Próby chronionych operacji</h3></div></header><div className="protected-activity-list">{protectedActivity.length ? protectedActivity.slice(0, 12).map((item, index) => <div key={`${item.clientCode}-${item.lastAt.getTime()}-${index}`} className={`severity-${item.severity}`}><span><strong>{item.label}</strong><small>Klient {item.clientCode} · {formatDate(item.lastAt)}</small></span><b>{item.count} prób</b></div>) : <p>Brak zapisanych prób w bieżącym oknie ochrony.</p>}</div></article>

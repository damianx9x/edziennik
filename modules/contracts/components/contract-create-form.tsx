...
export function ContractCreateForm({ parents, packages }: { parents: ParentOption[]; packages: PackageOption[] }) {
  ...
  return (
    ...
    {packages.length ? <ActionForm state={reuseState} action={reuseAction} className="stage4-form contract-package-reuse">
      ...
      <div className="stage4-form-row"><label>Rodzic<select name="parentId" required value={selectedParentId} onChange={(event) => { setSelectedParentId(event.target.value); setSelectedStudentId(""); }}><option value="" disabled>Wybierz rodzica</option>{visibleParents.map((parent) => <option key={parent.id} value={parent.id}>{parent.name}{parent.status === "INVITED" ? " · konto oczekuje" : ""}</option>)}</select></label><label>Uczeń<select name="studentId" required value={selectedStudentId} onChange={(event) => setSelectedStudentId(event.target.value)} disabled={!selectedParentId || children.length === 0}><option value="" disabled>{!selectedParentId ? "Najpierw wybierz rodzica" : children.length ? "Wybierz ucznia" : "Najpierw przypisz dziecko w Kartotekach"}</option>{children.map((child) => <option key={child.id} value={child.id}>{child.name}</option>)}</select></label><label>Termin odpowiedzi (opcjonalnie)<input name="expiresAt" type="date" /></label></div>
    ...
    <ActionForm state={state} action={action} className="stage4-form contract-package-form">
      ...
      <div className="stage4-form-row">
        <label>Rodzic<select name="parentId" required value={selectedParentId} onChange={(event) => { setSelectedParentId(event.target.value); setSelectedStudentId(""); }}><option value="" disabled>Wybierz rodzica</option>{visibleParents.map((parent) => <option key={parent.id} value={parent.id}>{parent.name}{parent.status === "INVITED" ? " · konto oczekuje" : ""}</option>)}</select></label>
